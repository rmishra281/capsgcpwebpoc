...
pipeline {
    ...
    environment {
        ...
        APP_NAME = "capsgcp"
        GCR_IMAGE = "gcr.io/${JENK_INT_IT_PROJECT_ID}/capsgcp:${BUILD_ID}"
    ...
    stages {
	    ...
	    steps {
			...
			sh "tar --exclude='./.git' -zcvf /tmp/$BUILD_CONTEXT ."
		        ...
		        step([$class: 'ClassicUploadStep', credentialsId: env.JENK_INT_IT_PROJECT_ID, bucket: "gs://${BUILD_CONTEXT_BUCKET}", pattern: env.BUILD_CONTEXT])
	...
	stage("Deploy to staging") {
		...
		container('gke-deploy') {
		    sh "sed -i s#IMAGE#${GCR_IMAGE}#g kubernetes/manifest.yaml"
                    step([$class: 'KubernetesEngineBuilder', projectId: env.JENK_INT_IT_PROJECT_ID, clusterName: env.STAGING_CLUSTER, location: env.PROJECT_ZONE, manifestPattern: 'kubernetes/manifest.yaml', credentialsId: env.JENK_INT_IT_PROJECT_ID, verifyDeployments: true])
		}
            }
	}
        /**
